...
- `docs/sonarqube-trend.md`: Dokumentation für SonarQube Quality Metrics Trend-Tracking — erklärt automatische Snapshots, vorausgesetzte Einrichtung (Automatic Analysis deaktivieren) und Branch-Vergleich bei PRs (#2)
...
- `.github/actions/sonarcloud`: GitHub Composite Action für SonarQube Analysis — lädt alle Coverage- und SARIF-Artefakte, arrangiert C-Reports, führt Scan durch; erweiterbar für künftige SARIF-Reports (#5–#8, #9)
...
- `strictdoc_to_sarif.py`: Konvertiert StrictDoc-Anforderungen ohne `TYPE: File` Relations in SARIF 2.1.0 — erscheinen als Code Smells in SonarQube und GitHub Code Scanning
...
- `.github/actions/python-security`: GitHub Composite Action für Python Security Scanning — bandit OWASP Scanner mit nativem SARIF-Output, Upload zu GitHub Code Scanning + SonarQube via `python-sarif` Artifact; Inputs: `scan-targets`, `bandit-args`, `upload-sarif`, `artifact-name`
